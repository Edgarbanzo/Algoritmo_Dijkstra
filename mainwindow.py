...
class MainWindow(QMainWindow):
    ...
    def dijkstra(self):
        ...
        self.adminparticula.crear_grafo_peso()
    
        origen_x = self.ui.origen_x_spinBox.value()
        origen_y = self.ui.origen_y_spinBox.value()
        inicio = (origen_x, origen_y)
        destino_x = self.ui.destino_x_spinBox.value()
        destino_y = self.ui.destino_y_spinBox.value()
        destino = (destino_x, destino_y)
        if Nodo(inicio) not in self.adminparticula.lista() or Nodo(destino) not in self.adminparticula.lista():
            ...
            distancias, camino = self.adminparticula.dijkstra(inicio)
            self.adminparticula.mostrar_dijkstra(distancias)
            self.adminparticula.mostrar_dijkstra(camino)
            self.scene.clear()
            pen = QPen()
            pen.setColor(QColor(255, 51, 205))
            pen.setWidth(3)
            auxDestino = Nodo(destino)
            while auxDestino != Nodo(inicio):
                actual = camino[auxDestino]
                self.scene.addLine(actual.get_x()+3, actual.get_y()+3, auxDestino.get_x()+3, auxDestino.get_y()+3, pen)
                auxDestino = camino[auxDestino]
    
    @Slot()
    def kruskal(self):
        self.ui.salida_plainTextEdit.clear()
        grafo = self.adminparticula.crear_grafo_velocidad()#le pasamos a la variable grafo el grafo con la velocidad
        kruskal = self.adminparticula.kruskal(grafo) #Mandamos a llamar la funcion de kruskal y le pasamos los parametros de nuestro grafo
        mostrar = self.grafo.mostrar_grafo(kruskal[1]) #Mandamos a llamar a la funcion que agregamos para poder imprimir el grafo
        
        print("Kruskal: ")
        print(mostrar)
        self.ui.salida_plainTextEdit.insertPlainText("Kruskal: "+"\n")
        self.ui.salida_plainTextEdit.insertPlainText(mostrar)
        pen = QPen()
        pen.setWidth(2)
        ...
        for nodo in kruskal[0]:
            origen = nodo[1]
            destino = nodo[2]
            self.scene.addLine(origen.get_x()+3, origen.get_y()+3, destino.get_x()+3, destino.get_y()+3, pen)#Agregamos la funcion getnodo en NODO
    ...
    def prim(self):
        ...
        for particula in self.adminparticula:
            ...
            self.grafo.agregar_arista(arista)
        if inicio not in self.grafo.get_lista_ady():
            QMessageBox.critical(
                self, "Error:", "No es posible leer los valores"
            )
        else:
            prim = self.grafo.prim(inicio)
            pen = QPen()
            pen.setWidth(2)
            color = QColor(255, 51, 205)
            pen.setColor(color)
    ...
    def action_mostrar_grafo(self):
        self.ui.salida_plainTextEdit.clear()
        grafo = self.adminparticula.crear_grafo()
        mostrar = self.adminparticula.mostrar_grafo_lista(grafo)
        self.ui.salida_plainTextEdit.insertPlainText(mostrar)
        self.dibujar()
